...
// ─── Create another Admin (admin-only, bypasses the first-run setup gate) ─
// Intended for use from inside the admin console. The /auth/register
// endpoint remains locked once any admin exists; this is the privileged
// path for provisioning additional administrators.
export const createAdmin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const hash = await bcrypt.hash(password, 12);
    const employee_id = await uniqueAdminEmployeeId();
...
async function uniqueAdminEmployeeId(): Promise<string> {
  for (let i = 0; i < 10; i++) {
