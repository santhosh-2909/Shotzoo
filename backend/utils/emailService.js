...
const sendEmployeeIdEmail = async (email, fullName, employeeId) => {
  // Skip email if credentials not configured
  if (!process.env.EMAIL_USER || process.env.EMAIL_USER === 'your_email@gmail.com') {
    console.log(`[Email skipped] Employee ID ${employeeId} for ${email} (configure EMAIL_USER in .env to enable)`);
    return false;
  }
  try {
    const mailOptions = {
    ...
    console.log(`Employee ID email sent to ${email}`);
    return true;
  } catch (error) {
...
// Send a one-time password used to gate "View Password" reveals on the profile page.
// Always logs the OTP to the server console too — that's the dev fallback for when
// EMAIL_USER/EMAIL_PASSWORD aren't configured yet, so testers can still complete the flow.
const sendOtpEmail = async (email, fullName, otp) => {
  console.log('[OTP] ' + email + ' (' + (fullName || 'user') + ') → ' + otp + ' (valid 5 min)');
  if (!process.env.EMAIL_USER || process.env.EMAIL_USER === 'your_email@gmail.com') {
    console.log('[Email skipped] OTP ' + otp + ' for ' + email + ' (configure EMAIL_USER in .env to enable)');
    return false;
  }
  try {
...
module.exports = { sendEmployeeIdEmail, sendOtpEmail };
