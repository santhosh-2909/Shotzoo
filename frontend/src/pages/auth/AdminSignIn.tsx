...
// Admin-only sign-in portal. Uses the same /api/auth/login endpoint
// but refuses non-admin accounts client-side and nudges them toward
// the employee sign-in page. Linked from /signup, /setup, and the
// SetupGate's "admin already exists" redirect.
...
export default function AdminSignIn() {
  ...
  // If already authenticated as an admin, skip the form.
  useEffect(() => {
    if (token && isAdmin) navigate('/admin/dashboard', { replace: true });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // Gate: refuse to log the user in through this portal unless they
      // are actually an admin. Employees should use /signin instead.
      if (!data.user.isAdmin) {
        setError('This portal is for admins only. Please use the employee sign in.');
        setPassword('');
        return;
      }
    ...
    <main className="flex min-h-screen w-full overflow-hidden bg-surface font-body text-on-surface animate-fade-in">
      ...
      <section className="hidden md:flex md:w-1/2 bg-[#0d0f08] flex-col justify-between p-12 relative overflow-hidden">
        <div className="z-10">
          ...
          <div className="mt-6 inline-flex items-center gap-2 py-1.5 px-3 rounded-full bg-primary-container/20 border border-primary-container/40">
      ...
      <section className="w-full md:w-1/2 bg-on-surface flex flex-col items-center justify-center p-6 md:p-12 relative">
        <div className="w-full max-w-md">
          <div className="md:hidden flex items-center gap-2 mb-12">
            ...
            <span className="text-2xl font-headline font-bold text-surface tracking-tighter">ShotZoo Admin</span>
          </div>

          <header className="mb-10 text-left">
            <div className="md:inline-flex hidden items-center gap-2 py-1.5 px-3 rounded-full bg-primary-container/10 mb-4">
              <span className="material-symbols-outlined text-[14px] text-primary-container">shield_person</span>
              <span className="text-[10px] font-bold uppercase tracking-widest text-primary-container">
                Admin Portal
              </span>
            ...
            <p className="text-surface-variant font-body font-medium">Sign in to your admin workspace</p>
          </header>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-2">
              <label className="block text-sm font-bold text-surface-container-highest font-label" htmlFor="admin-identity">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-bold text-surface-container-highest font-label" htmlFor="admin-password">
                Password
              </label>
              <div className="relative group">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-surface-variant group-focus-within:text-primary transition-colors">
            ...
            <footer className="pt-4 space-y-3 text-center">
              <p className="text-sm font-medium text-surface-container-highest">
                Need to create the first admin?{' '}
                <Link to="/setup" className="text-primary-container font-bold hover:underline ml-1">Set up workspace</Link>
              </p>
              <p className="text-sm font-medium text-surface-container-highest">
                Not an admin?{' '}
                <Link to="/signin" className="text-primary-container font-bold hover:underline ml-1">Employee sign in</Link>
              </p>
            </footer>
          </form>
        </div>
